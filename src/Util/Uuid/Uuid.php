...
namespace Tekkl\Shared\Util\Uuid;

use Tekkl\Shared\Util\Uuid\Exception\InvalidUuidException;
use Tekkl\Shared\Util\Uuid\Exception\InvalidUuidLengthException;
...
class Uuid
{
    ...
    {
        $ret = hex2bin(self::randomHex());
        if ($ret === false) {
            throw new \RuntimeException('Failed to generate random UUID');
        }
        return $ret;
    }
